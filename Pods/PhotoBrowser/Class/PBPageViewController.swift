...
public class PBPageViewController: UIPageViewController,UIPageViewControllerDataSource {
    ...
    fileprivate var initIndex : NSInteger = 0
...
//    override var prefersStatusBarHidden: Bool{return self.hideStatusBar}
   ...
   public init(sourceData :NSArray?,currentPhotoUrl:String,showStyle :PBStyle) {
        ...
        self.initIndex = (self.sourceData?.index(of: currentPhotoUrl))!
    ...
    fileprivate func initSubviews() {
        ...
        self.progressLabel.text = "\(self.initIndex + 1)/\((self.childArray?.count)!)"
    ...
    fileprivate func addChildViewControllers() {
       self.sourceData?.enumerateObjects({[unowned self] (object, index, _) in
        ...
        let currentController = self.childArray?.object(at: self.initIndex)
    ...
    @objc fileprivate func btnAction(sender :UIButton) {
        if sender.tag == saveBtnTag {
            if let childController  = self.viewControllers?.first as? PBChiledViewController {
